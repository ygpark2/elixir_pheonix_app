defmodule AinComBookingWeb.CompanyServiceLive do
  ...
  def render(assigns) do
    ...
    <.shell
      ...
      page_subtitle="Company services are your primary offerings. Configure manual and automatic slots directly from each service detail page."
    >
      <div class="space-y-5">
        ...
        <section :if={@live_action == :index} class="space-y-4">
          <div :if={@services == []} class="rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center text-sm text-slate-500">
            No company services yet. Create the first one and start opening slots for bookings.
          </div>
        ...
        <section :if={@live_action == :show and @service} class="space-y-4">
          ...
          <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 class="text-lg font-semibold tracking-tight text-slate-950">Service Slot Calendar</h3>
          ...
          <.modal
            ...
            id="service-manual-slot-modal"
            show={true}
            on_cancel={JS.push("close_manual_slot_modal")}
          >
            <% hour_stats = manual_hour_stats(@service_slots, @manual_slot_date) %>
            <div class="space-y-4">
              <div class="border-b border-slate-200 px-1 pb-4">
              ...
              >
                <div class="grid gap-4 md:grid-cols-2">
                  <div>
                    <.label for="service-manual-slot-date">날짜 (UTC)</.label>
                    <input
                      id="service-manual-slot-date"
                      type="date"
                  ...
                  <div>
                    <.label for="service-manual-slot-max-bookings">최대 예약 수(선택)</.label>
                    <input
                      id="service-manual-slot-max-bookings"
                      type="number"
                      min="1"
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50">
                  ...
                  <div
                    id="service-manual-slot-drag-grid"
      ...
      >
        <div class="space-y-4">
          <div class="border-b border-slate-200 px-1 pb-4">
            ...
            <h2 class="mt-2 pr-8 text-2xl font-semibold tracking-tight text-slate-950"><%= @bookings_modal_service_name || "Service" %> 예약 목록</h2>
          ...
          <div :if={@bookings_modal_bookings != []} class="max-h-[60vh] space-y-3 overflow-y-auto pr-1">
            <article :for={booking <- @bookings_modal_bookings} class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <.simple_form
                ...
                as={:booking}
                id={"service-booking-edit-form-#{booking.id}"}
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     ...
     |> assign(:bookings_modal_service_id, nil)
     |> assign(:bookings_modal_service_name, nil)
  ...
  def handle_event("open_service_bookings_modal", %{"service_id" => service_id}, socket) do
    case CompanyConsole.get_company_service(socket.assigns.current_user, service_id) do
      %CompanyService{} = service ->
        {:noreply,
         socket
         |> assign(:show_bookings_modal, true)
         |> assign(:bookings_modal_service_id, service.id)
         |> assign(:bookings_modal_service_name, service.name)
         |> assign(:bookings_modal_bookings, CompanyConsole.list_company_bookings_for_service(socket.assigns.current_user, service.id))
         |> assign(:editing_booking_id, nil)
         |> assign(:booking_edit_form, nil)}

      _ ->
        {:noreply, put_flash(socket, :error, "서비스를 찾을 수 없습니다.")}
    end
  end
  ...
  def handle_event("edit_booking", %{"booking_id" => booking_id}, socket) do
    case fetch_service_booking(socket, booking_id) do
      {:ok, booking} ->
        {:noreply,
         socket
  ...
  def handle_event("cancel_booking", %{"booking_id" => booking_id}, socket) do
    with {:ok, booking} <- fetch_service_booking(socket, booking_id),
         {:ok, _updated_booking} <- CompanyConsole.update_company_booking(socket.assigns.current_user, booking, %{"status" => "cancelled"}) do
      {:noreply,
       socket
       |> refresh_service_bookings_modal()
       |> maybe_refresh_service_slot_calendar()
  ...
  def handle_event("create_manual_slot", %{"manual_slot" => params}, socket) do
    ...
    with {:ok, max_bookings} <- parse_manual_slot_max_bookings(params["max_bookings"]),
      ...
      {created_count, skipped_count} =
        ...
        |> Enum.reduce({0, 0, existing_keys}, fn range, {created, skipped, seen_keys} ->
          ...
          if MapSet.member?(seen_keys, window_key) do
            ...
            attrs =
              manual_slot_attrs(socket.assigns.service, %{
      ...
      {:noreply,
       ...
       |> assign(:manual_slot_max_bookings, normalize_manual_slot_max_bookings(params["max_bookings"]))
       |> assign(:manual_slot_error, nil)
       |> assign_service_slot_state(socket.assigns.service,
  ...
  def handle_event("select_calendar_date", %{"date" => selected_date}, socket) do
    case Date.from_iso8601(selected_date) do
      {:ok, date} ->
        {:noreply,
         assign_service_slot_state(socket, socket.assigns.service,
           selected_date: date,
      ...
      service ->
        ...
        socket =
          if action == :show do
            ...
            |> assign(:show_manual_slot_modal, false)
            |> assign(:show_auto_slot_modal, false)
            |> assign_service_slot_state(service)
  ...
  defp assign_service_slot_state(socket, %CompanyService{} = service, opts \\ []) do
    slots = service_slots(socket.assigns.current_user, service.id)
  ...
  defp fetch_service_booking(socket, booking_id) do
    service_id = socket.assigns.bookings_modal_service_id

    case CompanyConsole.get_company_booking(socket.assigns.current_user, booking_id) do
      %CompanyBooking{service_id: ^service_id} = booking -> {:ok, booking}
      _ -> {:error, :not_found}
    end
  end

  defp refresh_service_bookings_modal(socket) do
    case socket.assigns.bookings_modal_service_id do
      service_id when is_binary(service_id) ->
        assign(
          socket,
          :bookings_modal_bookings,
          CompanyConsole.list_company_bookings_for_service(socket.assigns.current_user, service_id)
        )
  ...
  defp maybe_refresh_service_slot_calendar(socket) do
    if socket.assigns.live_action == :show and
         match?(%CompanyService{}, socket.assigns.service) and
         socket.assigns.service.id == socket.assigns.bookings_modal_service_id do
      assign_service_slot_state(socket, socket.assigns.service)
    else
      socket
    end
  ...
  defp service_slots(user, service_id) do
    slots =
      user
  ...
  defp manual_slot_attrs(%CompanyService{} = service, attrs) when is_map(attrs) do
    attrs = stringify_keys(attrs)
    default_start = Map.get(attrs, "start_time") || default_manual_slot_start()

    attrs
    |> Map.put_new("start_time", default_start)
    |> Map.put_new("end_time", default_manual_slot_end(default_start, service))
  ...
  defp default_manual_slot_end(start_time, %CompanyService{} = service) do
    duration_minutes =
  ...
  defp create_auto_slots_for_service(user, %CompanyService{} = service, existing_slots, changeset) do
    config = %{
