defmodule AinComBooking.CompanyConsole.SlotGeneration do
  @moduledoc false
  ...
  def resolve_booking_slot(_page, nil), do: {:error, :unavailable}

  def resolve_booking_slot(%BookingPage{} = page, slot_id) do
  ...
  def get_booking_service(%CompanySlot{service_id: nil}), do: {:ok, nil}

  def get_booking_service(%CompanySlot{service_id: service_id}) do
  ...
  def get_booking_resource(%CompanySlot{resource_id: nil}), do: {:ok, nil}

  def get_booking_resource(%CompanySlot{resource_id: resource_id}) do
  ...
  def ensure_slot_matches_page(%BookingPage{} = page, %CompanySlot{} = slot) do
    with :ok <- ensure_target_match(page.service_id, slot.service_id) do
      ensure_target_match(page.resource_id, slot.resource_id)
    end
  end

  def scope_booking_targets(%BookingPage{} = page, service, resource) do
    {
  ...
  def build_pricing(service, resource) do
  ...
  def maybe_close_full_slot(%CompanySlot{max_bookings: nil}, _booking_count_after), do: :ok

  def maybe_close_full_slot(%CompanySlot{} = slot, booking_count_after) do
  ...
  def validate_booking_status_capacity(changeset) do
  ...
  def sync_slot_status(slot_id) when is_binary(slot_id) do
    case Repo.get(CompanySlot, slot_id) do
      nil ->
        :ok
  ...
  def sync_slot_status(_slot_id), do: :ok
  ...
  defp validate_slot_service_belongs_to_user(changeset, %User{} = user) do
    case Ecto.Changeset.get_field(changeset, :service_id) do
      ...
      service_id ->
        if Inventory.get_company_service(user, service_id) do
          changeset
        else
  ...
  defp validate_slot_resource_belongs_to_user(changeset, %User{} = user) do
    case Ecto.Changeset.get_field(changeset, :resource_id) do
      ...
      resource_id ->
        if Inventory.get_company_resource(user, resource_id) do
          changeset
        else
  ...
  defp validate_slot_booking_page_belongs_to_user(changeset, %User{} = user) do
    case Ecto.Changeset.get_field(changeset, :booking_page_id) do
      ...
      booking_page_id ->
        if BookingPages.get_booking_page(user, booking_page_id) do
          changeset
        else
  ...
  defp build_auto_virtual_windows(%BookingPage{} = page, from_dt, to_dt) do
    with true <- page.auto_slots_enabled,
      ...
      timezone = BookingPages.company_timezone(page)
  ...
  defp apply_slot_page_targets(attrs, %User{} = user) do
    case map_get(attrs, "booking_page_id") do
      ...
      booking_page_id ->
        case BookingPages.get_booking_page(user, booking_page_id) do
          %BookingPage{} = page ->
            attrs
