defmodule AinComBookingGraphQL do
  @moduledoc false

  alias Absinthe.Phase.Document.Complexity.Result
  ...
  def configuration do
    [
      ...
      schema: AinComBookingGraphQL.Schema,
  ...
  def absinthe_pipeline(config, options) do
    ...
    |> Pipeline.insert_after(Result, {AbsintheSecurity.Phase.IntrospectionCheck, options})
    |> Pipeline.insert_after(Result, {AbsintheSecurity.Phase.MaxAliasesCheck, options})
    |> Pipeline.insert_after(Result, {AbsintheSecurity.Phase.MaxDepthCheck, options})
    |> Pipeline.insert_after(Result, {AbsintheSecurity.Phase.MaxDirectivesCheck, options})
