...
The project exposes a `GET /health` route that serves the `AinComHealth` module. This module contains checks to make sure the application and its external dependencies are healthy.
...
The project exposes a [Telemetry UI](https://github.com/mirego/telemetry_ui) dashboard through the `GET /metrics` route. Metrics are configured [here](lib/ain_com/telemetry_ui/telemetry_ui.ex).
