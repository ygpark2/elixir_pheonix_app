...
.PHONY: run_stop_db
run_stop_db: loadEnv ## Run stop Postgresql server
	docker compose down postgres-db

.PHONY: wait-for-db
wait-for-db:
	@echo "⏳ Waiting for PostgreSQL to become ready..."
	@until docker compose --profile db --env-file ./conf/docker.env exec -T postgres-db pg_isready -U root -d public; do \
...
.PHONY: test
test: loadEnv ## Run the test suite
	mix test
